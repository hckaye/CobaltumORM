...
コーディングエージェント向けの入口は[コーディングエージェント用ツール](docs/ai/agent-tools.ja.md)です。続けて[クイックリファレンス](docs/ai/quick-reference.ja.md)、[タスク別レシピ](docs/ai/recipes.ja.md)、[ビルド診断](docs/ai/diagnostics.ja.md)、索引の [llms.txt](llms.txt) を参照してください。
...
`schema` は、空のスキーマにすべてのマイグレーションの `Up` 定義を順番に反映し、最終的なテーブルと列を UTF-8 の JSON へ書き出します。データベースへの接続や connection string の読み込みは行いません。既定の出力先は、マイグレーションプロジェクト直下の `schema.generated.json` です。別の出力先は `--output` で指定できます。出力先の親ディレクトリがない場合は作成し、既存のファイルは置き換えます。JSON は 2 スペースでインデントし、日本語を含む識別子や式をそのまま表示します。テーブルとプロパティの順序を固定するため、差分レビューにも使えます。`formatVersion` は JSON の形式を表します。`schema` と `defaultExpression` に値がない場合は JSON の `null` を出力します。
...
`up` に `--write-schema` を付けると、マイグレーションが成功した後に同じ JSON を出力します。出力先は `--output` で変更できます。`--dry-run` と併用した場合はデータベースを変更せず、マイグレーション後に想定されるスキーマをファイルへ出力します。
