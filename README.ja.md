...
実行するたびにプロジェクト全体を解析します。結果のキャッシュはまだないため、1 回の実行にかかる時間は Query プロジェクトのフルビルド 1 回とおおむね同じです。このコストの削減は作業中です。現時点で通常の編集サイクルからこのコストを外す方法は、上記のように Query プロジェクトを分けることです。
