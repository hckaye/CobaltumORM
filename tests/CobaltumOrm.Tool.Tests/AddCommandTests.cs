...
public sealed class AddCommandTests
{
    ...
    public async Task AddsCobaltumOrmConfigurationToAnExistingProject()
    {
        ...
        Assert.Contains("<PackageReference Include=\"CobaltumOrm\" Version=\"0.0.6\" />", project, StringComparison.Ordinal);
        Assert.Contains("<PackageReference Include=\"CobaltumOrm.Migrations\" Version=\"0.0.6\" />", project, StringComparison.Ordinal);
        Assert.Contains("<PackageReference Include=\"CobaltumOrm.SourceGenerator\" Version=\"0.0.6\" PrivateAssets=\"all\" />", project, StringComparison.Ordinal);
    ...
    public async Task RejectsAConflictingPackageVersionBeforeChangingTheProject()
    {
        ...
        var migration = fixture.WriteMigration("Example.Database", "PostgreSql", "0.0.6");
        var before = File.ReadAllBytes(fixture.ApplicationProject);
        using var output = new StringWriter();
        using var error = new StringWriter();
    ...
    public async Task AddsPackageVersionsToNearestCentralPackageFile()
    {
        ...
        Assert.Contains("<PackageVersion Include=\"CobaltumOrm\" Version=\"0.0.6\" />", packages, StringComparison.Ordinal);
        Assert.Contains("<PackageVersion Include=\"CobaltumOrm.Migrations\" Version=\"0.0.6\" />", packages, StringComparison.Ordinal);
        Assert.Contains("<PackageVersion Include=\"CobaltumOrm.SourceGenerator\" Version=\"0.0.6\" />", packages, StringComparison.Ordinal);
    ...
    public async Task AcceptsAnExistingCompatibleCentralPackageVersion()
    {
        using var fixture = new AddFixture();
        fixture.WriteApplication("Example.App");
        fixture.WriteCentralPackages(("CobaltumOrm", "0.0.6"));
        var migration = fixture.WriteMigration("Example.Database", "PostgreSql");
        using var output = new StringWriter();
        using var error = new StringWriter();
    ...
    public async Task CreatesMigrationProjectUnderTheSameCentralPackageFile()
    {
        ...
        Assert.Contains("<PackageVersion Include=\"CobaltumOrm.Migrations\" Version=\"0.0.6\" />", packages, StringComparison.Ordinal);
        Assert.Contains("<PackageVersion Include=\"CobaltumOrm.SourceGenerator\" Version=\"0.0.6\" />", packages, StringComparison.Ordinal);
        Assert.Contains("<PackageVersion Include=\"CobaltumOrm.Migrations.Sqlite\" Version=\"0.0.6\" />", packages, StringComparison.Ordinal);
    ...
    public async Task CreatesMigrationProjectWithASeparateCentralPackageFile()
    {
        ...
        var migrationProject = File.ReadAllText(migration);
        Assert.Contains("<PackageVersion Include=\"CobaltumOrm\" Version=\"0.0.6\" />", targetPackages, StringComparison.Ordinal);
        Assert.Contains("<PackageVersion Include=\"Microsoft.Data.SqlClient\" Version=\"7.0.2\" />", targetPackages, StringComparison.Ordinal);
        Assert.Contains("<PackageVersion Include=\"CobaltumOrm.Migrations.SqlServer\" Version=\"0.0.6\" />", migrationPackages, StringComparison.Ordinal);
    ...
    public async Task CreationOutsideCentralManagementUsesTheTargetCentralDriverVersionExplicitly()
    {
        ...
        var project = File.ReadAllText(migration);
        Assert.Contains("<PackageReference Include=\"CobaltumOrm\" Version=\"0.0.6\" />", project, StringComparison.Ordinal);
        Assert.Contains("<PackageReference Include=\"CobaltumOrm.Migrations.PostgreSql\" Version=\"0.0.6\" />", project, StringComparison.Ordinal);
    ...
    public async Task UsesAnOlderConsistentCobaltumOrmVersionAlreadyInBothProjects()
    {
        ...
        Assert.DoesNotContain("Version=\"0.0.6\"", project, StringComparison.Ordinal);
    }
